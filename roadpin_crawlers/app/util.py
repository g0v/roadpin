...
def http_multipost(the_url_data, timeout=5):
    the_urls = the_url_data.keys()
    rs = (grequests.post(the_url, data=the_url_data[the_url], timeout=timeout) for the_url in the_urls)
...
def http_multiget(the_urls):
    ...
    try:
        ...
        cfg.logger.error('the_urls: %s', the_urls)
        result = {}
